...
class DistProcs():
    def __init__(self, n_actors, n_learners, mode, net_fp, args):
        self.args = args
        ###if train, load tsc types and weights?
        if mode == 'train':
        ...
        if args.load == True:
            net_data = load_data('net_data.p')
            self.net_data = net_data

        self.agents = [tsc for tsc in net_data['tsc']]
        ...
        exp_replay = manager.dict({ tsc: manager.list() for tsc in net_data['tsc']  })

        ###create list of desired proces, allocate agents evenly across learners
        actor_procs = [ActorProc(a, args, exp_replay, self.rl_stats, net_data, actor_eps[a], barrier) for a in range(n_actors)]
        learner_procs = [LearnerProc(l, args, exp_replay, self.rl_stats, net_data, learner_agents[l], barrier ) for l in range(n_learners)]
